...
function Card({ info }) {
  ...
  const {
    id, name, description, created_at, topics, html_url,
  } = info;
  ...
  return (
    <div className="card h-100 text-center text-bg-light" key={id} id={id}>
      ...
      <div className="card-body position-relative">
        ...
        <div className="d-flex flex-wrap justify-content-center gap-1 px-1">
          {topics.map((topic, i) => <span key={`span-${id}-${i}`} className="badge rounded-pill px-2 text-bg-info align-self-center">{ topic }</span>) }
        </div>
        <a href={html_url} className="position-absolute p-0" style={{ bottom: '0px', right: '0px' }}>
          <svg xmlns="http://www.w3.org/2000/svg" height="25" fill="currentColor" className="bi bi-github" viewBox="0 0 16 16">
